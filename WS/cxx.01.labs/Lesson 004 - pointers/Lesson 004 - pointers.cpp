...
int main()
{
    ...
    */
    ...
    /*
        A pointer variable stores a value which represents a memory location, but can also
        access the value stored at the memory location.
        i.e. the variable num has a value of 55, but variable num must also be located
        somewhere in memory, and a pointer variable allows access to the memory address.

        The pointer variable is always big enough to store the complete memory address,
        so on a 64-bit operating system, the pointer variable storing the address
        will be 64 bits in size.
