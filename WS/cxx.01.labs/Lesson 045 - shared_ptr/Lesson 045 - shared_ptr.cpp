...
// Provide thread safe queue which will push values up to max_value,
...
void ThreadLoopPush(std::shared_ptr<ThreadedQueue> threaded_queue)
{
    Print("Push started");
...
void ThreadLoopPop(std::shared_ptr<ThreadedQueue> threaded_queue)
{
    Print("Pop started");
...
int main()
{
    // The pointer managed by the shared_ptr object is thread safe, but any functions
    // accessed on the ThreadQueue will need thread protection.
    std::shared_ptr<ThreadedQueue> threaded_queue = std::make_shared<ThreadedQueue>();

    std::thread t1(ThreadLoopPush, threaded_queue);

    // There is no further need to have a reference to the shared pointer,
    // so it's safe to transfer the last reference to the shared pointer into the last
    // function call using it.
    std::thread t2(ThreadLoopPop, std::move(threaded_queue));
    t2.join();
    t1.join();
